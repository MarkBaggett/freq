...
parser.add_argument('-a','--add_exclude',default = "\n\t~`!@#$%^&*()_+-;", required=False,help='Provide a list of characters to add to default ignored characters.',dest='additional')
...
def score(line):
    if args.cut:
        try:
            ...
            raise(Exception("Invalid Cut specified. Must be a delimiter and field number."))
        if args.verbose: print("Now sorting on {}".format(line))
    if args.slice:
        try:
            line = line[int(args.slice[0]):int(args.slice[1])]
        except:
            raise(Exception("Invalid Substring specified. Must be a start and stop integer position of the string."))
        if args.verbose: print("Now sorting on {}".format(line))
    if args.numeric:
        try:
            return float(line)
        except:
            raise(Exception("The value {} can not be treated as numeric".format(line)))
